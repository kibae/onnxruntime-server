...
onnxruntime_server::transport::http::https_session::~https_session() {
	try {
		...
		PLOG(L_WARNING) << get_remote_endpoint() << " transport::http::https_session::~https_session: " << e.what()
						<< std::endl;
	}
}
...
void onnxruntime_server::transport::http::https_session::on_read(beast::error_code ec, std::size_t bytes_transferred) {
	...
	if (ec) {
		PLOG(L_WARNING) << get_remote_endpoint() << " transport::http::https_session::do_read: " << ec.message()
						<< std::endl;
		return close();
	}
